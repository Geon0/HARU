...
<%MemberDto member = (MemberDto) session.getAttribute("member"); 
		if(member == null){
			response.sendRedirect("sign.do?redirect=board.do?command=writeform");
		}else{
			if( !(member.getIsclass().equals("AD"))){
				response.sendRedirect("board.do?command=error");
			}
		}
%>
<body>
        ...
        <div class="container">
            ...
            <div class="navbar-right">
                <ul class="nav navbar-nav">
                    ...
                    <li class="nav-item topLi" style="margin: 20PX;"><a class= "menulink" href="profile.do?command=mypage" style="color: black;">마이페이지</a>
  ...
  <div class="board_table_detail" style="padding-left:350px;">
   ...
   <form action="board.do?command=write" method="post">
    <table class="table_deta" border= "1px solid rgb(170, 168, 168);">
          ...
          <tr>
              <th>제목</th>
              <td colspan="2"><input type="text" name="title" size="92"></td>
          </tr>
          <tr>
              <th>내용</th>
              <td colspan="2"><textarea rows="15" cols="94" name="content" style="resize: none;"></textarea></td>
          </tr>
    </table>
        <div class="board_btn" style="padding: 20px 280px;">
          <input type="submit" class="btn btn-outline-primary" value="글쓰기" style="outline: none; background-color:rgb(129, 129, 129); color:white;" >
          
          <button type="button" class="btn btn-outline-primary" onclick="location.href='board.do?command=list'" style="outline: none;">취소</button>
        </div>
    </form>
